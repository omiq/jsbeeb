...
async function loadHTMLFile(file) {
    ...
    // Auto-boot the dropped disc (simulate SHIFT+BREAK)
    // Add a delay to ensure the disc is fully loaded and emulator is ready
    setTimeout(() => {
        console.log("Auto-booting dropped disc:", file.name);
        
        // Ensure the emulator is running before autobooting
        if (!running) {
            console.log("Starting emulator for autoboot");
            go();
        }
        
        // Wait for keyboard to be initialized
        if (!keyboard) {
            console.log("Waiting for keyboard to be initialized...");
            setTimeout(() => autoboot(file.name), 200);
        } else {
            autoboot(file.name);
        }
    }, 100);
...
function autoboot(image) {
    ...
    console.log("Autobooting disc:", image);
    console.log("Keyboard initialized:", !!keyboard);
    console.log("Emulator running:", running);
    utils.noteEvent("init", "autoboot", image);

    // Simulate JavaScript keyboard events for SHIFT+F12 (SHIFT+BREAK)
    console.log("Simulating SHIFT+F12 keyboard events for autoboot");
    
    // Create and dispatch SHIFT keydown event
    ...
    document.dispatchEvent(shiftKeyDown);
    
    // Wait a bit, then send F12 (BREAK) keydown
    setTimeout(() => {
        const f12KeyDown = new KeyboardEvent('keydown', {
            key: 'F12',
            keyCode: 123,
            which: 123,
            shiftKey: true,
        ...
        document.dispatchEvent(f12KeyDown);
        
        // Wait a bit, then release F12
        setTimeout(() => {
            const f12KeyUp = new KeyboardEvent('keyup', {
                key: 'F12',
                keyCode: 123,
                which: 123,
                shiftKey: true,
            ...
            document.dispatchEvent(f12KeyUp);
            ...
            setTimeout(() => {
                ...
                document.dispatchEvent(shiftKeyUp);
            }, 50);
        }, 50);
    }, 50);
}
